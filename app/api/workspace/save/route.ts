...
async function loadWorkspaceIndex(userId: string): Promise<WorkspaceIndexEntry[]> {
  const url = getDownloadUrl({ bucket: WORKSPACES_BUCKET, key: indexKey(userId), expiresInSeconds: 60 })
...
export async function POST(req: Request) {
  ...
  try {
    const body = (await req.json()) as { name?: unknown; data?: unknown }
    ...
    // Validate that the data is JSON (string). We store it as-is.
    try {
      JSON.parse(data)
    } catch {
      return NextResponse.json({ error: "Invalid request: data must be valid JSON string" }, { status: 400 })
    }

    const workspaceId = `ws_${Date.now().toString(36)}_${crypto.randomBytes(5).toString("hex")}`
    ...
    const nextIndex = [{ id: record.id, name: record.name, created_at: record.created_at, updated_at: record.updated_at }, ...index]
