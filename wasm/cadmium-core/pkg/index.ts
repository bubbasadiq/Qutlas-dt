...
//
// Note: In this repo we ship a lightweight JS implementation (cadmium_core.js) to ensure
// geometry creation works everywhere (including environments where compiling Rust/WASM
// is not available). This module provides stable, ergonomic exports used by tests and
// higher-level code.
...
export type OpLogEntry =
  ...
  | { type: "create-torus"; params: { majorRadius: number; minorRadius: number; segmentsMajor?: number; segmentsMinor?: number } }
...
export function createTorus(majorRadius: number, minorRadius: number, segmentsMajor = 32, segmentsMinor = 16) {
...
export function applyParameters(_mesh: any, params: { length?: number; width?: number; height?: number }) {
  ...
  for (const entry of opLog) {
    switch (entry.type) {
      ...
      case "create-cylinder":
        ...
        break
      case "create-sphere":
        current = createSphere(entry.params.radius, entry.params.segmentsLat, entry.params.segmentsLon)
        break
      case "create-cone":
      ...
      case "boolean":
        ...
        // we simply no-op.
        if (!entry.params.other) break
        if (entry.params.operation === "union") current = boolean_union(current, entry.params.other)
        if (entry.params.operation === "subtract") current = boolean_subtract(current, entry.params.other)
        if (entry.params.operation === "intersect") current = boolean_intersect(current, entry.params.other)
        break
      default:
        break
    }
