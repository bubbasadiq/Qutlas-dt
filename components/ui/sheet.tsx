        ...
        <div className="fixed inset-0 z-50">
          ...
          <SheetContent onClose={() => onOpenChange(false)}>
            {React.Children.toArray(children).find(
              (child: any) => child?.type?.name === 'SheetBody'
            )}
          </SheetContent>
        </div>
      )}
...
export function SheetContent({ children, onClose, className }: SheetContentProps) {
  return (
    ...
    >
      {/* Handle bar */}
      <div
        className="w-12 h-1.5 bg-gray-300 rounded-full mx-auto mt-3 mb-2"
        onClick={onClose}
      />
      {children}
    </div>
...
export function SheetHeader({ children, className }: SheetHeaderProps) {
  return (
    <div className={cn("px-4 pt-2 pb-3 border-b border-gray-100", className)}>
      {children}
    </div>
  )
}
